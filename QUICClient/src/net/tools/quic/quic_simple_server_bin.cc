...
int main(int argc, char* argv[]) {
  ...
  if (line->HasSwitch("h") || line->HasSwitch("help")) {
    const char* help_str =
        ...
        "--fill_cache_at_startup   files are loaded to memory at startup\n";
    std::cout << help_str;
    exit(0);
  }
  ...
  if (line->HasSwitch("quic_response_cache_dir")) {
	  if (line->HasSwitch("fill_cache_at_startup")) {
	     response_cache.InitializeFromDirectory(line->GetSwitchValueASCII("quic_response_cache_dir"));
	   }
  }
