...
QuicHttpResponseCache::ServerPushInfo::ServerPushInfo(QuicUrl request_url, SpdyHeaderBlock headers, SpdyPriority priority, string body) :
		request_url(request_url), headers(std::move(headers)), priority(priority), body(body) {
}

QuicHttpResponseCache::ServerPushInfo::ServerPushInfo(const ServerPushInfo& other) :
		request_url(other.request_url), headers(other.headers.Clone()), priority(other.priority), body(other.body) {
}
...
QuicHttpResponseCache::ResourceFile::ResourceFile(const base::FilePath& file_name) :
...
void QuicHttpResponseCache::ResourceFile::Read() {
	...
	while (start < file_contents_.length()) {
		...
		if (pos == string::npos) {
			QUIC_LOG(DFATAL) << "Headers invalid or empty, ignoring: " << file_name_.value();
			return;
		}
		...
		spdy_headers_.AppendValueOrAddHeader(QuicTextUtils::ToLower(line.substr(0, pos)), line.substr(pos + 2));
	}
	...
	if (it != spdy_headers_.end()) {
		...
		while (start < push_urls.length()) {
			...
			if (pos == string::npos) {
				push_urls_.push_back(QuicStringPiece(push_urls.data() + start, push_urls.length() - start));
				break;
			}
	...
	body_ = QuicStringPiece(file_contents_.data() + start, file_contents_.size() - start);
}

void QuicHttpResponseCache::ResourceFile::SetHostPathFromBase(QuicStringPiece base) {
...
QuicStringPiece QuicHttpResponseCache::ResourceFile::RemoveScheme(QuicStringPiece url) {
...
const QuicHttpResponseCache::Response* QuicHttpResponseCache::GetResponse(QuicStringPiece host, QuicStringPiece path) {

	base::CommandLine* line = base::CommandLine::ForCurrentProcess();

	auto it = responses_.find(GetKey(host, path));

	if (it == responses_.end()) { //if response not found in cache

		DVLOG(1) << "Response not found in cache, FindResponseAndAddToCache will be called : host " << host << " path " << path;
		FindResponseAndAddToCache(host, path, line->GetSwitchValueASCII("quic_response_cache_dir"));

		it = responses_.find(GetKey(host, path));

		if (it == responses_.end()) {
			DVLOG(1) << "Get response for resource failed: host " << host << " path " << path;
	...
	DVLOG(1) << "Response found in cache : host " << host << " path " << path;
	return it->second.get();
}

typedef QuicHttpResponseCache::ServerPushInfo ServerPushInfo;

void QuicHttpResponseCache::AddSimpleResponse(QuicStringPiece host, QuicStringPiece path, int response_code, QuicStringPiece body) {
...
void QuicHttpResponseCache::AddSimpleResponseWithServerPushResources(QuicStringPiece host, QuicStringPiece path, int response_code,
		QuicStringPiece body, std::list<ServerPushInfo> push_resources) {
...
void QuicHttpResponseCache::AddResponse(QuicStringPiece host, QuicStringPiece path, SpdyHeaderBlock response_headers,
...
void QuicHttpResponseCache::AddResponse(QuicStringPiece host, QuicStringPiece path, SpdyHeaderBlock response_headers,
		QuicStringPiece response_body, SpdyHeaderBlock response_trailers) {
	AddResponseImpl(host, path, REGULAR_RESPONSE, std::move(response_headers), response_body, std::move(response_trailers));
}

void QuicHttpResponseCache::AddSpecialResponse(QuicStringPiece host, QuicStringPiece path, SpecialResponseType response_type) {
	AddResponseImpl(host, path, response_type, SpdyHeaderBlock(), "", SpdyHeaderBlock());
}

QuicHttpResponseCache::QuicHttpResponseCache() {
}

void QuicHttpResponseCache::FindResponseAndAddToCache(QuicStringPiece host, QuicStringPiece path, const string& cache_dir) {
	if (cache_dir.empty()) {
		QUIC_BUG << "cache_directory must not be empty. ";
		return;
	}
	QUIC_LOG(INFO) << "\n FindResponseAndAddToCache Parameters: " << "\n host: " << host
			       << "\n path: " << path << "\n cache_dir: " << cache_dir;

	std::list<std::unique_ptr<ResourceFile>> resource_files;


	string full_file_path = cache_dir + path.as_string();

	QuicStringPiece file_path_str(full_file_path);

	QUIC_LOG(INFO) << "\n file_path_str:  " << file_path_str;

	FilePath file_iter(FilePath::FromUTF8Unsafe(file_path_str));
	...
	base.remove_prefix(cache_dir.length());
	if (base[0] == '/') {
	...
	AddResponse(resource_file->host(), resource_file->path(), resource_file->spdy_headers().Clone(), resource_file->body());
	...
	for (const auto& resource_file : resource_files) {
		...
		for (const auto& push_url : resource_file->push_urls()) {
			...
			push_resources.push_back(ServerPushInfo(url, response->headers().Clone(), kV3LowestPriority, response->body().as_string()));
		}
		MaybeAddServerPushResources(resource_file->host(), resource_file->path(), push_resources);
	}

}

void QuicHttpResponseCache::InitializeFromDirectory(const string& cache_directory) {
	if (cache_directory.empty()) {
		QUIC_BUG << "cache_directory must not be empty.";
		return;
	}
	QUIC_LOG(INFO) << "Attempting to initialize QuicHttpResponseCache from directory: " << cache_directory;
	...
	std::list<std::unique_ptr<ResourceFile>> resource_files;
	for (FilePath file_iter = file_list.Next(); !file_iter.empty(); file_iter = file_list.Next()) {
...
std::list<ServerPushInfo> QuicHttpResponseCache::GetServerPushResources(string request_url) {
	QuicWriterMutexLock lock(&response_mutex_);
	...
	QUIC_DVLOG(1) << "Found " << resources.size() << " push resources for " << request_url;
	return resources;
}
...
void QuicHttpResponseCache::AddResponseImpl(QuicStringPiece host, QuicStringPiece path, SpecialResponseType response_type,
		SpdyHeaderBlock response_headers, QuicStringPiece response_body, SpdyHeaderBlock response_trailers) {
...
string QuicHttpResponseCache::GetKey(QuicStringPiece host, QuicStringPiece path) const {
	return host.as_string() + path.as_string();
}

void QuicHttpResponseCache::MaybeAddServerPushResources(QuicStringPiece request_host, QuicStringPiece request_path,
		std::list<ServerPushInfo> push_resources) {
	...
	for (const auto& push_resource : push_resources) {
		...
		QUIC_DVLOG(1) << "Add request-resource association: request url " << request_url << " push url "
								<< push_resource.request_url.ToString() << " response headers " << push_resource.headers.DebugString();
		{
			QuicWriterMutexLock lock(&response_mutex_);
		...
		if (!found_existing_response) {
			...
			QUIC_DVLOG(1) << "Add response for push resource: host " << host << " path " << path;
			AddResponse(host, path, push_resource.headers.Clone(), body);
		}
	}
}

bool QuicHttpResponseCache::PushResourceExistsInCache(string original_request_url, ServerPushInfo resource) {
	QuicWriterMutexLock lock(&response_mutex_);
	auto resource_range = server_push_resources_.equal_range(original_request_url);
	for (auto it = resource_range.first; it != resource_range.second; ++it) {
		ServerPushInfo push_resource = it->second;
		if (push_resource.request_url.ToString() == resource.request_url.ToString()) {
			return true;
		}
	}
	return false;
